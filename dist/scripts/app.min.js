...
if(!targetId)return;var timeoutPromise=$timeout(function(){var spy=new Spy(targetId,$scope,$element,-($attr.offset?parseInt($attr.offset,10):duScrollOffset));spyAPI.addSpy(spy);$scope.$on('$locationChangeSuccess',spy.flushTargetCache.bind(spy));var deregisterOnStateChange=$rootScope.$on('$stateChangeSuccess',spy.flushTargetCache.bind(spy));$scope.$on('$destroy',function(){spyAPI.removeSpy(spy);deregisterOnStateChange();});},0,false);$scope.$on('$destroy',function(){$timeout.cancel(timeoutPromise);});}};}]);angular.module('ng-touch',[]).directive('touch',function(){return{restrict:'C',link:function(scope,element,attrs){element.on("click",function(event){var elem,d,x,y;if(element.find(".touch-elem").length===0){element.prepend("<div class='touch-elem'></div>   ");}
...
{for(var i=0;i<objectArray.length;i++)
{objectArray[i][0].style.left=objectArray[i][1]+"px";}}});app.controller('404Ctrl',function($scope){analytics.page({path:'/404'});});app.controller('aproposCtrl',function($scope){analytics.page({path:'/apropos'});});app.controller('contactCtrl',function($scope){analytics.page({path:'/contact'});});app.controller('formCtrl',function($cookies,$scope,$timeout){$scope.data={sector:"",email:"",fname:"",lname:"",phone:"",company:""};$scope.zipCode="";$scope.isGetPositionLoading=false;$scope.isLastStepReached=false;$scope.countTo=340;$scope.startTime=moment(new Date());$scope.step=1;$scope.maxStep=5;$scope.totalStep=5;$scope.isLoadingFinished=false;$scope.getPosition=function(){var geocoder=new google.maps.Geocoder;var zip="";console.log(navigator);$scope.isGetPositionLoading=true;if(navigator.geolocation){navigator.geolocation.getCurrentPosition(function(position){var pos={lat:position.coords.latitude,lng:position.coords.longitude};geocoder.geocode({'location':pos},function(results,status){if(status===google.maps.GeocoderStatus.OK){if(results[1]){console.log(results);var concat="";for(var i=0;i<results.length;i++){concat+=results[i].formatted_address;}
console.log($scope.zipCode);$scope.zipCode=extractZipCode(concat);$scope.$applyAsync();$scope.isGetPositionLoading=false;}else{console.log("No result found");}}else{console.log('Geocoder failed due to: '+status);}});},function(){$.getJSON("http://ip-api.com/json/?callback=?",function(data){$scope.zipCode=data.zip;$scope.$applyAsync();$scope.isGetPositionLoading=false;});});}}
...
{$scope.step=number;updateBar();updateNav();}};$scope.next=function(form){if($scope.step==$scope.maxStep){$scope.endTime=moment(new Date());var duration=moment.duration($scope.endTime.diff($scope.startTime));console.log(duration.format("h:mm:ss"));$scope.duration=duration.format("h:mm:ss");}
if(!form.$invalid){$scope.step++;if($scope.maxStep<$scope.step)
$scope.maxStep++;if($scope.maxStep==$scope.step){$scope.isLastStepReached=true;$timeout(function(){$scope.isLoadingFinished=true;},125500);}
updateBar();updateNav();}};var theText=$("#theText");var theOutputText=$("#theOutputText");var theOutputKeyPress=$("#theOutputKeyPress");var theOutputKeyDown=$("#theOutputKeyDown");var theOutputKeyUp=$("#theOutputKeyUp");var theOutputFocusOut=$("#theOutputFocusOut");theText.keydown(function(event){keyReport(event,theOutputKeyDown);});theText.keypress(function(event){keyReport(event,theOutputKeyPress);});theText.keyup(function(event){keyReport(event,theOutputKeyUp);});theText.focusout(function(event){theOutputFocusOut.html(".focusout() fired!");});theText.focus(function(event){theOutputFocusOut.html(".focus() fired!");});function keyReport(event,output){if(event.which==10||event.which==13){event.preventDefault();}
output.html(event.which+"&nbsp;&nbsp;&nbsp;&nbsp;event.keyCode "+event.keyCode);switch(event.which){case 0:output.append("event.which not sure");break;case 13:output.append(" Enter");break;case 27:output.append(" Escape");break;case 35:output.append(" End");break;case 36:output.append(" Home");break;}
theOutputText.text(theText.val());}});app.controller('homeCtrl',function($cookies,$scope,mouseParallaxService,$timeout){$scope.setCookie=false;var favoriteCookie=$cookies.get('cookiePopup');if(favoriteCookie=='true')
