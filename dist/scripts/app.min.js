...
$scope.next=function(){console.log($scope.data);if($scope.step==$scope.totalStep){$scope.endTime=moment(new Date());var duration=moment.duration($scope.endTime.diff($scope.startTime));console.log(duration.format("h:mm:ss"));$scope.duration=duration.format("h:mm:ss");}
else if($scope.forms[$scope.step-1].$valid){$scope.step++;if($scope.maxStep<$scope.step)
...
rsa=0;console.log("RSA FINAL",rsa);$scope.finalResult.rsa=rsa;$scope.finalResult.apl=apl;$scope.$applyAsync();};}).directive('watchChange',function(){return{scope:{onchange:'&watchChange'},link:function(scope,element,attrs){element.on('input',function(){console.log(1);scope.onchange();});}};});app.controller('homeCtrl',function($cookies,$scope,mouseParallaxService,$timeout){$scope.setCookie=false;var favoriteCookie=$cookies.get('cookiePopup');if(favoriteCookie=='true')
