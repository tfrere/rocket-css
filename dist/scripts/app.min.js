...
{for(var i=0;i<objectArray.length;i++)
{objectArray[i][0].style.left=objectArray[i][1]+"px";}}});app.controller('404Ctrl',function($scope){analytics.page({path:'/404'});});app.controller('aproposCtrl',function($scope){analytics.page({path:'/apropos'});});app.controller('blogCtrl',function($scope,$http,$sce,$timeout){$timeout=twttr.widgets.load();analytics.page({path:'/blog'});$scope.trackNewsletter=function(){window.analytics.track('NewsLetter add');};$http.get('../ressources/blog.json').success(function(data){var articles=[];for(var i=0;i<data.length;i++){var content=data[i].content;var title=data[i].title;var date=data[i].date;var tags=data[i].tags;var img=data[i].img;content=$sce.trustAsHtml(content);articles.push({index:i,content:content,title:title,date:date,tags:tags,img:img});}
