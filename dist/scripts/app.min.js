...
mouseParallaxService.init();$scope.trackCta1=function(){window.analytics.track('cta1');};$scope.trackCta2=function(){window.analytics.track('cta2');};});app.controller('navCtrl',function($scope,$rootScope){$scope.trackCreate=function(){window.analytics.track('Account Created');};$rootScope.$on('$stateChangeError',function(event){$state.go('404');});});app.controller('questionAppCtrl',function($scope,$http,$sce){analytics.page({path:'/faq/app'});$scope.trackQuestion=function(index){window.analytics.track('question '+index);};$http.get('../ressources/question_app.json').success(function(data){var questions=[];for(var i=0;i<data.length;i++){var content=data[i].content;var title=data[i].title;content=$sce.trustAsHtml(content);questions.push({index:i,content:content,title:title});}
$scope.questions=questions;});});app.controller('questionStatutCtrl',function($scope,$http,$sce){analytics.page({path:'/faq/statut'});$scope.trackQuestion=function(index){window.analytics.track('question '+index);};$http.get('../ressources/question_statut.json').success(function(data){var questions=[];for(var i=0;i<data.length;i++){var content=data[i].content;var title=data[i].title;content=$sce.trustAsHtml(content);questions.push({index:i,content:content,title:title});}
