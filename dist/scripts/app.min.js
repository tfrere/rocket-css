...
{console.log(form.email);button.addClass('on');window.analytics.track('Question submitted');$http.post('http://app.peon.fr/api/questions/',{email:form.email,name:form.name,msg:form.content}).success(function(){button.removeClass('on');$scope.isSended=true;$scope.isGood=true;console.log('YEAH');}).error(function(){button.removeClass('on');$scope.isSended=true;$scope.isGood=false;console.log('NOES');});}};});app.controller('blogCtrl',function($scope,$http,$sce,$timeout){$timeout(function(){$timeout=twttr.widgets.load();},100);analytics.page({path:'/blog'});$scope.trackNewsletter=function(){window.analytics.track('NewsLetter add');};$http.get('https://app.peon.fr/api/posts').success(function(data){var articles=[];for(var i=0;i<data.length;i++){var content=data[i].content.substring(0,408);var title=data[i].title;var date=moment(data[i].publishedAt).format("Do MMMM YYYY");var RawDate=data[i].publishedAt;var tags=data[i].tags;var img=data[i].img;var slug=data[i].slug;var isMore=false;if(data[i].content.length>400)
