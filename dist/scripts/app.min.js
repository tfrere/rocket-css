...
{console.log(form.email);button.addClass('on');window.analytics.track('Question submitted');$http.post('http://app.tfrere.fr/api/question/',{email:form.email,name:form.name,msg:form.content}).success(function(){button.removeClass('on');$scope.isSended=true;$scope.isGood=true;console.log('YEAH');}).error(function(){button.removeClass('on');$scope.isSended=true;$scope.isGood=false;console.log('NOES');});}};});app.controller('blogCtrl',function($scope,$http,$sce,$timeout){$timeout=twttr.widgets.load();analytics.page({path:'/blog'});$scope.trackNewsletter=function(){window.analytics.track('NewsLetter add');};$http.get('http://app.peon.fr/api/posts').success(function(data){var articles=[];console.log(data);for(var i=0;i<data.length;i++){var content=data[i].content;var title=data[i].title;var date=data[i].date;var tags=data[i].tags;var img=data[i].img;content=$sce.trustAsHtml(content);articles.push({index:i,content:content,title:title,date:date,tags:tags,img:img});}
...
{console.log(form.email);button.addClass('on');window.analytics.track('Question submitted');$http.post('http://app.tfrere.fr/api/question/',{email:form.email,name:form.name,msg:form.content}).success(function(){button.removeClass('on');$scope.isSended=true;$scope.isGood=true;console.log('YEAH');}).error(function(){button.removeClass('on');$scope.isSended=true;$scope.isGood=false;console.log('NOES');});}};});app.controller('homeCtrl',function($cookies,$scope,mouseParallaxService,$timeout){$scope.setCookie=false;var favoriteCookie=$cookies.get('peonCookie');console.log(favoriteCookie);if(favoriteCookie=='true')
