...
if(!targetId)return;var timeoutPromise=$timeout(function(){var spy=new Spy(targetId,$scope,$element,-($attr.offset?parseInt($attr.offset,10):duScrollOffset));spyAPI.addSpy(spy);$scope.$on('$locationChangeSuccess',spy.flushTargetCache.bind(spy));var deregisterOnStateChange=$rootScope.$on('$stateChangeSuccess',spy.flushTargetCache.bind(spy));$scope.$on('$destroy',function(){spyAPI.removeSpy(spy);deregisterOnStateChange();});},0,false);$scope.$on('$destroy',function(){$timeout.cancel(timeoutPromise);});}};}]);var app=angular.module('app',['ngCookies','ui.router','ng-optimizely','duScroll','duParallax','toggleHeight','ngResource','ngAnimate']);app.run(['optimizely',function(optimizely){optimizely.loadProject('3529320510');}]);app.filter('reverse',function(){return function(items){return items.slice().reverse();};});app.directive('updateTitle',['$rootScope','$timeout',function($rootScope,$timeout){return{link:function(scope,element){var listener=function(event,toState){var title='Default Title';if(toState.data&&toState.data.pageTitle)title=toState.data.pageTitle;$timeout(function(){element.text(title+" | Peon");},0,false);};$rootScope.$on('$stateChangeSuccess',listener);}};}]);app.config(function($stateProvider,$urlRouterProvider,$locationProvider){$locationProvider.html5Mode({enabled:true,requireBase:false});$urlRouterProvider.when('','/');$locationProvider.hashPrefix('!');$urlRouterProvider.otherwise('/404');$stateProvider.state('nav',{abstract:true,templateUrl:'templates/nav.html',controller:'navCtrl'}).state('nav.home',{url:'/',templateUrl:'templates/home.html',controller:'homeCtrl',data:{pageTitle:'Home'}}).state('nav.faq',{url:'/faq',templateUrl:'templates/faq.html',controller:'faqCtrl',data:{pageTitle:'Faq'}}).state('nav.faq.show',{url:'/question/:id',templateUrl:'templates/question.html',controller:'questionCtrl',data:{pageTitle:'Faq'}}).state('nav.apropos',{url:'/apropos',templateUrl:'templates/apropos.html',controller:'aproposCtrl',data:{pageTitle:'A propos'}}).state('nav.assistance',{url:'/assistance',templateUrl:'templates/assistance.html',controller:'assistanceCtrl',data:{pageTitle:'Assistance'}}).state('nav.blog',{url:'/blog',templateUrl:'templates/blog.html',controller:'blogCtrl',data:{pageTitle:'Blog'}}).state('nav.article',{url:'/blog/article/:slug',templateUrl:'templates/article.html',controller:'articleCtrl',data:{pageTitle:'Article'}}).state('nav.cgu',{url:'/cgu',templateUrl:'templates/cgu.html',data:{pageTitle:'Cgu'}}).state('nav.404',{url:'/404',templateUrl:'templates/404.html',controller:'404Ctrl',data:{pageTitle:'404'}}).state('nav.cookies',{url:'/cookies',templateUrl:'templates/cookies.html',data:{pageTitle:'Cookies'}})});app.service('mouseParallaxService',function(){var IE=document.all?true:false
...
{for(var i=0;i<objectArray.length;i++)
{objectArray[i][0].style.left=objectArray[i][1]+"px";}}});app.controller('404Ctrl',function($scope){analytics.page({path:'/404'});});app.controller('aproposCtrl',function($scope){analytics.page({path:'/apropos'});});app.controller('articleCtrl',function($stateParams,$scope,$http,$sce,$timeout){console.log($stateParams);$http.get('http://app.peon.fr/api/posts/slug/'+$stateParams.slug).success(function(data){var article={};article=data;article.content=$sce.trustAsHtml(data.content);article.date=moment(data.publishedAt).format('YYYY');$scope.article=article;console.log(data);});analytics.page({path:'/blog/'+$stateParams.slug});});app.controller('assistanceCtrl',function($scope,$http){analytics.page({path:'/assistance'});$scope.isGood=null;$scope.isSended=false;var button=angular.element(document.getElementById('validate'));$scope.sendQuestion=function(form){if(form.$valid)
{console.log(form.email);button.addClass('on');window.analytics.track('Question submitted');$http.post('http://app.peon.fr/api/questions/',{email:form.email,name:form.name,msg:form.content}).success(function(){button.removeClass('on');$scope.isSended=true;$scope.isGood=true;console.log('YEAH');}).error(function(){button.removeClass('on');$scope.isSended=true;$scope.isGood=false;console.log('NOES');});}};});app.controller('blogCtrl',function($scope,$http,$sce,$timeout){$timeout(function(){$timeout=twttr.widgets.load();},100);analytics.page({path:'/blog'});$scope.trackNewsletter=function(){window.analytics.track('NewsLetter add');};$http.get('http://app.peon.fr/api/posts').success(function(data){var articles=[];console.log(data);for(var i=0;i<data.length;i++){var content=data[i].content.substring(0,408);var title=data[i].title;var date=data[i].date;var tags=data[i].tags;var img=data[i].img;var slug=data[i].slug;content=$sce.trustAsHtml(content);articles.push({index:i,slug:slug,content:content,title:title,date:date,tags:tags,img:img});}
$scope.articles=articles;});$scope.isGood=null;$scope.isSended=false;$scope.sendEmail=function(form){if(form.$valid)
{console.log(form.email);window.analytics.track('NewsLetter add');$http.post('/subscribeToMailchimp/',{email:form.email}).success(function(){$scope.isSended=true;$scope.isGood=true;console.log('YEAH');});}};});app.controller('contactCtrl',function($scope){analytics.page({path:'/contact'});});app.controller('faqCtrl',function($sce,$stateParams,$scope,$http,$document){analytics.page({path:'/faq'});$scope.moveTo=function(id){var offset=100;var duration=500;var someElement=angular.element(document.getElementById(id));$document.scrollToElement(someElement,offset,duration);}
...
{console.log(form.email);button.addClass('on');window.analytics.track('Question submitted');$http.post('http://app.peon.fr/api/questions/',{email:form.email,name:form.name,msg:form.content}).success(function(){button.removeClass('on');$scope.isSended=true;$scope.isGood=true;console.log('YEAH');}).error(function(){button.removeClass('on');$scope.isSended=true;$scope.isGood=false;console.log('NOES');});}};analytics.page({path:'/faq/'+$stateParams.id});$scope.trackQuestion=function(index){window.analytics.track('questionApp '+index);};console.log($stateParams);$http.get('http://app.peon.fr/api/infos').success(function(data){var questions=[];for(var i=0;i<data.length;i++){var content=data[i].content;var title=data[i].title;content=$sce.trustAsHtml(content);if(data[i].category===$stateParams.id)
questions.push({index:i,content:content,title:title});}
$scope.questions=questions;console.log(questions);});});app.controller('homeCtrl',function($cookies,$scope,mouseParallaxService,$timeout){$scope.setCookie=false;var favoriteCookie=$cookies.get('peonCookie');console.log(favoriteCookie);if(favoriteCookie=='true')
$scope.setCookie=true;$scope.updateCookie=function(){$scope.setCookie=true;var expireDate=new Date();expireDate.setDate(expireDate.getDate()+365);$cookies.put('peonCookie','true',{'expires':expireDate});console.log(favoriteCookie);};$scope.isVideoClicked=false;$scope.videoClickMask=function(){$scope.isVideoClicked=true;window.analytics.track('videoOpened');}
mouseParallaxService.init();$scope.trackCta1=function(){window.analytics.track('cta1');};$scope.trackCta2=function(){window.analytics.track('cta2');};});app.controller('navCtrl',function($scope,$rootScope){$scope.trackCreate=function(){window.analytics.track('Account Created');};$rootScope.$on('$stateChangeError',function(event){$state.go('404');});$scope.seo={};$scope.seo.title="Peon";$scope.seo.desc="Assistant personnel pour auto-entrepreneur. Il vous accompagne dans la gestion de vos clients, devis, factures, calcul de vos cotisations et traitement de vos impayés.";$scope.seo.keywords="peon, devis, facture, auto entrepreneur, comptabilité, prestataire de service, auto entreprenariat, impayés, signature electronique, libéral, artisan, s'inscrire, démarche création";});app.controller('questionCtrl',function($sce,$stateParams,$scope,$http,$document){$scope.moveTo=function(id){var offset=100;var duration=500;var someElement=angular.element(document.getElementById(id));$document.scrollToElement(someElement,offset,duration);}
