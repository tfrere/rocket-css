...
$scope.maxStep++;if($scope.maxStep==$scope.step){$scope.isLastStepReached=true;$timeout(function(){$scope.isLoadingFinished=true;},2500);}
...
$scope.slider_ticks={minValue:50,value:50,options:{ceil:80,floor:30,showTicks:true,step:35}};$scope.dateUpdate=function(){$scope.isEndOfLastJob=true;};$scope.sendForm=function(form){if(form.$valid)
{$http.post('/postForm',keepData).success(function(a,b){console.log(a,b);}).error(function(a,b){console.log(a,b);});}};}).directive('watchChange',function(){return{scope:{onchange:'&watchChange'},link:function(scope,element,attrs){element.on('input',function(){console.log(1);scope.onchange();});}};});app.controller('homeCtrl',function($cookies,$scope,mouseParallaxService,$timeout){$scope.setCookie=false;var favoriteCookie=$cookies.get('cookiePopup');if(favoriteCookie=='true')
